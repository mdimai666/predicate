...
if ($show_total_size) {
	list($sizeCurrentFolder,$fileCurrentNum,$foldersCurrentCount) = folder_info(curr($current_path),false);
}
/***
*SUB-DIR CODE
***/
...
if (($ftp && !$ftp->isDir($ftp_base_folder.$upload_dir.$rfm_subfolder.$subdir)) || (!$ftp && !file_exists(curr($current_path).$rfm_subfolder.$subdir)))
{
	$subdir = '';
	$rfm_subfolder = "";
}
...
if (trim($rfm_subfolder) == "")
{
	...
	$cur_path			= curr($current_path).$subdir;
...
else
{
	...
	$cur_path			= curr($current_path).$rfm_subfolder.$subdir;
...
if(!$ftp){
	...
	while($cycle && $i < $max_cycles){
		...
		if (file_exists(curr($current_path).$parent."config.php"))
		{
			require_once curr($current_path).$parent."config.php";
			$cycle = FALSE;
		}
...
if($ftp){
	...
	$files	= scandir(curr(($current_path)).$rfm_subfolder.$subdir);
	// $files	= scandir(curr($current_path).$rfm_subfolder.$subdir);
}
...
foreach($files as $k=>$file){
	if($ftp){
		...
		if($file!="." && $file!=".."){
			if(is_dir(curr($current_path).$rfm_subfolder.$subdir.$file)){
				$date=filemtime(curr($current_path).$rfm_subfolder.$subdir. $file);
				$current_folders_number++;
				if($show_folder_size){
					list($size,$nfiles,$nfolders) = folder_info(curr($current_path).$rfm_subfolder.$subdir.$file,false);
				} else {
					$size=0;
				}
				$file_ext=trans('Type_dir');
				...
				$current_files_number++;
				// $file_path=curr($current_path).$rfm_subfolder.$subdir.$file;
				$file_path= curr(($current_path)).$rfm_subfolder.$subdir.$file;
...
<!-- header div end -->
	...
	<div class="span12">
		<?php if( ($ftp && !$ftp->isDir($ftp_base_folder.$upload_dir.$rfm_subfolder.$subdir))  || (!$ftp && @opendir((curr($current_path)).$rfm_subfolder.$subdir)===FALSE)){ ?>
		<br/>
		...
		foreach ($files as $file_array) {
			...
			if($ftp && $file!='..' && $file!=$new_name){
				//rename
				rename_folder(curr($current_path).$subdir.$file,$new_name,$ftp,$config);
				$file=$new_name;
			}
			...
			foreach ($files as $nu=>$file_array) {
				...
				if(!$ftp){
					...
					if($file!=fix_filename($file,$config)){
						...
						$file_path1=(curr($current_path).$rfm_subfolder.$subdir.$file1);
						if(file_exists($file_path1)){
							$i = 1;
							$info=pathinfo($file1);
							while(file_exists(curr($current_path).$rfm_subfolder.$subdir.$info['filename'].".[".$i."].".$info['extension'])) {
								$i++;
							}
					...
					if($ftp){
						...
						if(!file_exists($src_thumb) ){
							if(!create_img($file_path, $creation_thumb_path, 122, 91,'crop',$ftp,$config)){
								...
								new_thumbnails_creation(curr($current_path).$rfm_subfolder.$subdir,$file_path,$file,curr($current_path),'','','','','','','',$fixed_image_creation,$fixed_path_from_filemanager,$fixed_image_creation_name_to_prepend,$fixed_image_creation_to_append,$fixed_image_creation_width,$fixed_image_creation_height,$fixed_image_creation_option);
							}
						}
						...
						if($img_width<45 && $img_height<38){
							$mini_src=curr($current_path).$rfm_subfolder.$subdir.$file;
